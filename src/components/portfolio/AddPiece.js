...
const AddPiece = (props) => {
    ...
    const addPiecePage = auth.uid ? <AddPieceForm /> : <SignInForm />;
    const addPieceTitle = auth.uid ? 'Upload A New Piece' : 'Authentication Required';
    const addPieceDesc = auth.uid ? 'Fill in the fields below to upload a new piece' : 'Please enter the email and password in order to add an article to this site.'
